...
//change showLog to see log in console
const showLog = false;
...
const MediumClap = () => {
  ...
  // medium clap render log  
  useEffect(()=>{
   showLog && console.log('medium clap render, refs=', clap, clapCount, clapCountTotal);
  });
  ...
  return (
    <div style={{width:'100%'}}>
      <button ref={clap} className={styles.clap} onClick={handleClapClick}>
        ...
        <ClapCount ref={clapCount} count={count}/>
        <ClapTotal ref={clapCountTotal} total={totalCount} />
      </button>
      <h3>02.js  useRef + customHook + useEffect</h3>
    </div>
  )
}
const ClapIcon = ({ isClicked }) => {
  useEffect(()=>{
   showLog && console.log('clap icon render');
  })
...
const ClapCount = React.forwardRef(({ count }, ref) => {
  useEffect(()=>{
   showLog && console.log('clap count render');
  })

  return <span ref={ref} className={styles.count}>+{count}</span>
});

const ClapTotal = React.forwardRef(({ total }, ref) => {
  useEffect(()=>{
   showLog && console.log('clap total render');
  }) 
  return <span ref={ref} className={styles.total}>{total}</span>
});
